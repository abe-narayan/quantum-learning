 ...
 * (17 x 0.5522 = 9.39px), and every string below is written to fit at 17.
 *
  ...
  return (
    <div className="not-prose overflow-x-auto panel-inset p-4">
      <svg width={WIDTH} height={HEIGHT} viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full" role="img" aria-label={ariaLabel}>
        {/* 41 characters = 418 units at 17, centred on 230, so 21..439 inside the
            460-unit box. */}
        <text x={WIDTH / 2} y={20} textAnchor="middle" fontSize={17} className="fill-muted-foreground font-mono">
          crosstalk: the drive leaks to a neighbour
        </text>
        ...
        {/* The two lines that state the quantitative claim the lesson then works
            out, so they are the last thing that should be painting at 6.6px. 42 and
            31 characters = 428 and 316 units at 17, both left-anchored at x=14. */}
        <text x={CHIP_X} y={232} fontSize={17} className="fill-muted-foreground font-mono">
          the leak reaches the nearest neighbour only
        </text>
        <text x={CHIP_X} y={256} fontSize={17} className="fill-muted-foreground font-mono">
          fidelity cost: F(ε) = cos&sup2;(ε/2)
        </text>
