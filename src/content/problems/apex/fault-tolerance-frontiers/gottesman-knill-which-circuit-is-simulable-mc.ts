...
export const gottesmanKnillWhichCircuitIsSimulableMc: MultipleChoiceProblem = {
  ...
  answer: {
    ...
    correctOptionId: "a",
    optionFeedback: {
      b: "One $T$ gate is one gate too many for this theorem. Gottesman-Knill's guarantee covers stabilizer circuits, and $T$ is outside the Clifford group, so the tableau bookkeeping the proof relies on no longer closes. Extended stabilizer-rank methods do still handle a circuit with a handful of $T$ gates, at a cost that climbs with the T-count, but that is a different result with a different guarantee, and it is exactly why T-count became the resource a fault-tolerant estimate is built around.",
