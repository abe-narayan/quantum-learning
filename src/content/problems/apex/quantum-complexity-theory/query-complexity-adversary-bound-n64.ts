...
export const queryComplexityAdversaryBoundN64: NumericProblem = {
  ...
  hints: [
    ...
    { text: "For each position i, Gamma ∘ D_i keeps only the entry where the YES instance differs from all-zero at bit i. That is exactly one entry, equal to 1, so its spectral norm is 1 for every i, giving max_i ||Gamma ∘ D_i|| = 1." },
  ],
  solution: {
    steps: [
      ...
      { description: "ADV(OR_64) = ||Gamma|| / max_i ||Gamma ∘ D_i|| = 8 / 1 = 8, matching sqrt(64), as the general formula ADV(OR_N) = sqrt(N) predicts." },
    ],
