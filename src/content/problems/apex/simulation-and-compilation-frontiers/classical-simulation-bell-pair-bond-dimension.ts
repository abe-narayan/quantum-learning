...
export const classicalSimulationBellPairBondDimension: NumericProblem = {
  ...
  question: {
    type: "numeric",
    prompt:
      "Extend the lesson's Bell-pair-crossing circuit from 6 qubits (3 crossing pairs) to n=10 qubits: apply H to qubits 0-4, then CNOT(i, i+5) for each i=0..4, entangling each of qubits 0-4 with its own independent partner in 5-9. This is Clifford-only, like the lesson's worked example. What is the bond dimension χ needed for an exact matrix-product-state representation across the cut separating {0,...,4} from {5,...,9}?",
  ...
  answer: {
    ...
    tolerance: 0.5,
    incorrectFeedback:
      "Each of the 5 independent Bell pairs contributes exactly 1 ebit of entanglement entropy across the cut (the same argument the lesson ran for 3 pairs, reaching 3 ebits). With 5 independent pairs the total entropy is 5 ebits, and bond dimension is χ=2^(entropy in ebits), not the entropy itself and not the qubit count.",
    nearMisses: [
      {
        value: numberOfPairs,
  ...
  hints: [
    ...
    { text: "Because the 5 pairs are mutually independent, the reduced state on {0,...,4} is the tensor product of 5 maximally mixed single-qubit states: the I/8 pattern the lesson verified numerically, generalized to I/32." },
  ...
  solution: {
    steps: [
      ...
      { description: "The 5 pairs are built from 5 disjoint qubit pairs with no gates coupling different pairs, so the reduced state on {0,...,4} factors as a tensor product: ρ_A = (I/2)^⊗5 = I/32, the maximally mixed state on 5 qubits, by the same argument the lesson verified numerically for 3 pairs (ρ_A = I/8)." },
  ...
  explanation: {
    correctIdea:
      "A Clifford circuit can push entanglement all the way to the maximum a bipartition allows. Gottesman-Knill is indifferent to that, since it simulates by tracking the stabilizer group algebraically, never the entanglement structure directly. The same state, viewed through the tensor-network lens, needs the maximum possible bond dimension for that cut.",
    whyCorrect:
