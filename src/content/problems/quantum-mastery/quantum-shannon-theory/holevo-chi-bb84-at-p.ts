...
export const holevoChiBb84AtP: NumericProblem = {
  ...
  explanation: {
    correctIdea:
      "The Holevo quantity subtracts the noise's own contribution to the individual output states' mixedness from the total spread of the ensemble's average state. What is left over is the label-distinguishing information a measurement could in principle extract.",
    whyCorrect:
      "This matches the capstone's computed curve: chi falls from about 0.601 bits at p=0 (noiseless, but still capped below 1 bit since |0> and |+> are non-orthogonal) through 0.286 bits at p=0.2, and continues toward 0 at p=1, since more depolarizing noise both raises each individual output's entropy and pulls the average state's entropy toward, but never below, that same value.",
    whyWrong: [
