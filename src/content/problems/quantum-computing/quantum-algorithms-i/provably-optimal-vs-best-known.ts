...
export const provablyOptimalVsBestKnown: MultipleChoiceProblem = {
  ...
  question: {
    ...
    options: [
      { id: "a", text: "Grover's Ω(√N) bound is proven against every quantum algorithm; Deutsch-Jozsa's gap is tied to its promise" },
      { id: "b", text: "Deutsch-Jozsa's speedup is the proven optimal one; Grover's Ω(√N) is only the best bound found so far" },
      { id: "c", text: "Both are proven optimal, each against every possible algorithm for its own problem" },
      { id: "d", text: "Neither result has been proven; both rest on how the algorithms behave when run" },
    ],
  },
  answer: {
    type: "multiple-choice",
    correctOptionId: "a",
    optionFeedback: {
      b: "This reverses the two results: Grover's bound is the one proven optimal.",
      ...
      d: "Both results are mathematical theorems, derived and, in Grover's case, proven optimal. Neither rests on experiment.",
    },
