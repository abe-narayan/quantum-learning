...
export const connectingBackToShorsAlgorithm: MultipleChoiceProblem = {
  ...
  question: {
    ...
    options: [
      { id: "a", text: "Shor's period-finding circuit must succeed in essentially one long, uninterrupted run, with no classical feedback loop to average out errors" },
      { id: "b", text: "Shor's algorithm doesn't use qubits, so error correction is irrelevant to it" },
      { id: "c", text: "VQE is immune to all physical errors by design" },
      { id: "d", text: "There is no real difference — both need identical error-correction resources" },
    ],
  },
  answer: {
    type: "multiple-choice",
    correctOptionId: "a",
    optionFeedback: {
      b: "Shor's algorithm is a genuine quantum circuit built from qubits and gates — this option is simply false.",
      c: "No quantum algorithm is immune to physical errors — VQE is only more structurally tolerant of them, not immune.",
  ...
  hints: [
    { text: "Shor's period-finding circuit is one long sequence that must work correctly essentially all at once." },
