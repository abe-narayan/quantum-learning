...
export const phaseOnQubitZeroConcentration: NumericProblem = {
  ...
  question: {
    ...
    inputHint: "as a decimal between 0 and 1; predict from the lesson's Worked Example before computing",
  },
  answer: {
    type: "numeric",
    ...
    tolerance: 0.01,
    nearMisses: [
      { value: 0.25, tolerance: 0.01, feedback: "0.25 is the weight of each outcome after the first H⊗H, before the phase and the second layer of Hadamards recombine them. The second layer is what interferes those four amplitudes back onto a single outcome." },
      { value: 0, tolerance: 0.005, feedback: "0 is P(|10⟩) for the version of this circuit with the Z on qubit 1, which concentrates on |01⟩ instead. The prompt moves the Z to the other qubit, which moves the certain outcome with it." },
    ],
    incorrectFeedback:
  ...
  hints: [
    ...
    { text: "Apply the second $H\\otimes H$ factor by factor, using $H|-\\rangle=|1\\rangle$ and $H|+\\rangle=|0\\rangle$. Which single basis state does the product land on, and is it the one the question asks about?" },
  ],
  solution: {
    steps: [
